...
void Section::add_literal(const std::string& literal) {
    pool.push_back({data.size(), -1, literal});
}
...
void Section::dumpPool() {
    for (auto& pair : sections) {
        ...
        for (auto& literal : section->pool) {
            ...
            if (literal.value == -1) SymTab::add_occurrence(literal.symbol, section_name, literal.line);
