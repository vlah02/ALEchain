...
void LinkerSymTab::parse_symbols_and_relocations(const std::vector<std::string>& filenames) {
    ...
    auto looks_int = [](const std::string& s) -> bool {
    ...
    auto to_int_or = [](const std::string& s, int base, int defval) -> int {
        if (s.empty()) return defval;
        char* end = nullptr;
        long v = std::strtol(s.c_str(), &end, base);
        if (end == s.c_str() + static_cast<long>(s.size())) return static_cast<int>(v);
        return defval;
    };
    ...
    for (const auto& fname : filenames) {
        ...
        while (true) {
            ...
            if (in_symbols) {
                ...
                int numOcc = 0;
                if (!toks.empty() && looks_int(toks.back())) {
                    numOcc = to_int_or(toks.back(), 0, 0);
                    toks.pop_back();
                }

                if (toks.empty()) {
                    std::cerr << "Error parsing symbols for '" << name << "': missing strength/type tokens\n";
                    exit(4);
                }
                ...
                if (strength == "weak") entry.is_weak = true;
                    ...
                    std::cerr << "Error: symbol '" << name << "' invalid strength token '" << strength << "'\n";
                    exit(4);
                }

                entry.stype = toks.empty() ? std::string("NOTYPE") : toks.front();
                if (!toks.empty()) toks.erase(toks.begin());

                bool have_loc = (toks.size() >= 2) && looks_int(toks[1]);
                std::string raw_section = have_loc ? toks[0] : "";
                int raw_offset = have_loc ? to_int_or(toks[1], 0, 0) : 0;
            ...
            if (in_relocations) {
                ...
                rel.file = fname;
                if (rel.section.empty() || rel.type.empty() || rel.symbol.empty()) {
                    std::cerr << "[WARN] Skipping malformed relocation line in " << fname << ": " << line << "\n";
                    continue;
                }
...
void LinkerSymTab::patch_relocations() {
    for (const auto& rel : LinkerSymTab::relocations) {
        auto symIt = symbol_values.find(rel.symbol);
        if (symIt == symbol_values.end()) {
            std::cerr << "[SKIP] Relocation: symbol '" << rel.symbol << "' has no resolved value!\n";
            continue;
        }
        int value = symIt->second + rel.addend;

        auto secIt = LinkerSections::merged_sections.find(rel.section);
        if (secIt == LinkerSections::merged_sections.end()) {
            std::cerr << "[SKIP] Relocation: section not found '" << rel.section << "'\n";
            continue;
        }
        auto& data = secIt->second.data;

        int patch_offset = rel.offset + LinkerSections::get_offset(rel.section, rel.file);

        std::cerr << "[PATCH_RELOC] "
                  << "section='" << rel.section << "' "
                  << "file='" << rel.file << "' "
                  << "offset_in_section=" << rel.offset << " "
                  << "resolved=0x" << std::hex << value << std::dec << " "
                  << "patch_offset=0x" << std::hex << patch_offset << std::dec
                  << " type=" << rel.type
                  << "\n";
        ...
        if (rel.type == "R_ABS32") {
            if (patch_offset + 3 < (int)data.size()) {
                ...
                std::cerr << "    Old 32b: 0x" << std::hex << oldval << std::dec << "\n";
                ...
                int newval = data[patch_offset] | (data[patch_offset+1] << 8)
                           | (data[patch_offset+2] << 16) | (data[patch_offset+3] << 24);
                std::cerr << "    New 32b: 0x" << std::hex << newval << std::dec << "\n";
            } else {
                std::cerr << "[ERROR] Patch offset out of range for relocation on symbol '"
                          << rel.symbol << "' at offset " << patch_offset
                          << " (data size: " << data.size() << ")\n";
            }
        } else {
            std::cerr << "[WARN] Relocation type '" << rel.type << "' not implemented; skipping.\n";
        }
    }
}
